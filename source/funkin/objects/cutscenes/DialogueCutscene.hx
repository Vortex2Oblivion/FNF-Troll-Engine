...
class DialogueCutscene extends Cutscene {
	...
	public override function createCutscene() 
	{
		...
		dialogueText = new Textbox(box.textOffsets[0], box.textOffsets[1], {
			textFieldWidth: box.textWidth,
			fontSize: 32,
			font: Paths.font(box.font),
			color: FlxColor.fromString(box.textColor),
		});
		dialogueText.statusChangeCallbacks.push(
			(status) -> {
				if (status == DONE) {
					finishLine();
				}
				else if(status == FULL){
					dialogueText.continueWriting();
				}
			}
		);
		dialogueText.characterDisplayCallbacks.push((text) -> {
			text.antialiasing = box.antialiasing;
			text.borderStyle = SHADOW;
			text.alignment = LEFT;
			text.borderSize = box.shadowWidth;
			text.borderColor = FlxColor.fromString(box.shadowTextColor);
			if(sounds.length > 0){
	...
	public function createNewLine() {
		...
		getTextSound();
		@:privateAccess
		dialogueText.settings.fontSize = getTextSize(curDialogueLine.text_size);
		dialogueText.setText(curDialogueLine.text);
		dialogueText.startWriting();
		dialogueText.bring();
	}
	...
	private function getTextSound() {
		...
		if (dialogueTalkSound != null)
			sounds = [
				for (dialogueSound in dialogueTalkSound) FlxG.sound.load(Paths.sound(dialogueSound), 0.6)
			];
	}
