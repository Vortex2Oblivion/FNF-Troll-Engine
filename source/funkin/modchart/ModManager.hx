...
class ModManager {
	...
	public function updateActiveMods(player:Int) {
		...
		for (mod in modArray) {
			...
			if (active_mods.contains(mod_name) && !mod.shouldExecute(player, mod.getValue(player))) {
				...
				if (mod.shouldExecute(player, mod.getValue(player))) {
					if (mod.parent != null
						&& !activated_mods.contains(mod.parent.getName())
						&& !active_mods.contains(mod.parent.getName()))
					...
					if (node.out_mods.length > 0) {
						if (output.length < node.out_mods.length) {
							for (i in (output.length - 1)...node.out_mods.length)
								output.push(node.in_mods.contains(node.out_mods[i]) ? getValue(node.out_mods[i],
									player) : 0); // If input mods contains the output mod, use the current value, else use 0.
						}
					}
	...
	public function getPos(diff:Float, tDiff:Float, beat:Float, data:Int, player:Int, obj:NoteObject, field:NoteField, ?exclusions:Array<String>,
			?pos:Vector3):Vector3 {
		if (!obj.alive)
			return pos;
		...
		var speed:Float = if (obj.objType == NOTE) getNoteSpeed(cast obj, player,
			field.songSpeed); else getCMod(data, player, field.songSpeed) * getXMod(data, player);

		diff += getValue("transformPath", player);
		diff += getValue("movePath", player) * FlxMath.lerp(Note.swagWidth, Conductor.crotchet * Note.pixelsPerMS * speed, getValue("movePathType", player));

		pos.setTo(Note.halfWidth + field.field.getBaseX(data), Note.halfWidth + 50 + diff, 0);
	...
	public function modifyVertex(beat:Float, vert:Vector3, idx:Int, obj:NoteObject, pos:Vector3, player:Int, data:Int, field:NoteField,
			?exclusions:Array<String>):Vector3 {
		if (!obj.active)
			return vert;
	...
	public function getExtraInfo(diff:Float, tDiff:Float, beat:Float, ?info:RenderInfo, obj:NoteObject, player:Int, data:Int,
			?exclusions:Array<String>):RenderInfo {
		if (!obj.active)
			return info;
	...
	public function queueEase(step:Float, endStep:Float, modName:String, target:Float, style:Any, player:Int = -1, ?startVal:Float) {
		...
		if (style == null) {} else if (style is String) {
